...
jest.mock('../src/components/game/map-next/UniverseMap', () => {
  return {
    __esModule: true,
    default: (props: any) => {
...
jest.mock('../src/stores/universeMapStore', () => {
  return {
    __esModule: true,
    default: () => ({
      selectedCoordinate: {
...
describe('GalaxyPage (client pkg): URL/store sync with ?view=…', () => {
  beforeEach(() => {
  ...
  test('initializes Next map from ?view=region and updates store+URL on onViewChange', async () => {
    const GalaxyPage = (await import('../src/components/game/GalaxyPage')).default;
