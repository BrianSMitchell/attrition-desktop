 ...
 * ============================================================
 *
 * Extends the base Jest setup to support comprehensive balance and performance
 * testing with appropriate timeouts, setup/teardown procedures, and reporting.
 *
 * This configuration is specifically tuned for:
 * - Long-running balance tests
 * - Performance benchmarking
 * - Complex game state scenarios
 * - Resource-intensive simulations
 *
 * Usage:
 * - npm test -- --config jest.config.balance-performance.ts
 */
...
const config: Config = {
  ...
  globalSetup: '<rootDir>/src/__tests__/setup/balance-performance-setup.ts',
  globalTeardown: '<rootDir>/src/__tests__/setup/balance-performance-teardown.ts',
  ...
  // Module paths and aliases (extends base config)
  moduleNameMapper: {
    ...(baseConfig.moduleNameMapper as Record<string, string>),
  ...
  testResultsProcessor: '<rootDir>/src/__tests__/processors/balance-performance-processor.ts',
