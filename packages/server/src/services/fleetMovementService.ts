...
export class FleetMovementService {
   ...
   */
  static calculateFleetSpeed(units: FleetUnit[]): number {
  ...
  static async dispatchFleet(
    ...
    try {
      ...
      if (!fleet.units || fleet.units.length === 0 || fleet.units.every((u: any) => u.count === 0)) {
        return {
          success: false,
      ...
      if (insertError) {
        console.error('Error creating fleet movement:', insertError);
        return {
          success: false,
  ...
  static async getFleetStatus(
    ...
    try {
      ...
      const fleetWithDetails = {
        ...
        units: fleet.units.map((unit: any) => {
          const unitSpec = getUnitSpec(unit.unitKey as any);
          return {
  ...
  private static async processArrival(movement: any): Promise<void> {
    try {
      ...
      if (fleetUpdateError) {
        console.error('Error updating fleet location:', fleetUpdateError);
        return;
      }
      ...
      if (movementUpdateError) {
        console.error('Error updating movement status:', movementUpdateError);
        return;
      }
