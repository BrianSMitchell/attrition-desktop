...
import { SupabaseCapacityService } from './SupabaseCapacityService';

/**
 * SupabaseBaseCitizenService - maintains citizen accrual per base.
 * Uses aligned payout periods with milli-citizens remainder to avoid fractional loss,
 * similar to Empire credits.
 */
export class SupabaseBaseCitizenService {
  ...
  static async updateEmpireBases(empireId: string): Promise<{ updated: number; errors: number }> {
    ...
    try {
      ...
      if (fetchError) {
        console.error('[SupabaseBaseCitizenService] Error fetching colonies:', fetchError);
      ...
      for (const c of colonies || []) {
        try {
          ...
          if (updateError) {
            console.error(`[SupabaseBaseCitizenService] Error updating colony ${c.id}:`, updateError);
            errors++;
            continue;
          }
          ...
          errors++;
          console.error('[SupabaseBaseCitizenService] update colony error', e);
        }
      }
    } catch (e) {
      console.error('[SupabaseBaseCitizenService] top-level error', e);
    }
