...
import { ResourceCalculationService } from '../../services/resources/ResourceCalculationService';
import { getOnlineUniqueUsersCount } from '../../services/socketService';

/**
 * DashboardController handles dashboard-related business logic
 * Extracted from the dashboard route to maintain separation of concerns
 */
export class DashboardController {
   ...
   */
  static createNewPlayerResponse(user: any): DashboardData {
    return {
      user,
      empire: null,
      resourcesGained: 0,
  ...
  /**
   * Formats dashboard response for API consumption
   * @param data - The dashboard data to format
   * @returns ApiResponse - Formatted API response
   */
  static formatDashboardResponse(data: DashboardData): ApiResponse {
    return {
      success: true,
      data
    };
