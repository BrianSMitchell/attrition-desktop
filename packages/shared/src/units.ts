...
// Union of all unit keys (normalize to snake_case)
export type UnitKey =
  | 'fighter'
  | 'bomber'
  | 'heavy_bomber'
...
export interface UnitTechPrereq {
  key: TechnologyKey;
  level: number;
}
...
export interface UnitSpec {
  ...
  // Energy delta while active (negative = consumes energy). Optional for Phase A if unknown.
  energyDelta?: number;

  // Movement/propulsion type as displayed in UI. Allow literal annotations like "Stellar*".
  drive?: DriveType | string;

  // Primary weapon type
  weapon?: WeaponType;

  // Display-only combat/ship stats (not enforced in Phase A)
  attack?: number;
  armour?: number;
  shield?: number;
  // Hangar capacity contribution:
  //   negative = consumes hangar space (e.g., -1 for Fighters),
  //   positive = provides hangar space (e.g., 80 for Carrier),
  //   0 = none
  hangar?: number;
  // Travel speed (relative scalar shown in UI)
  speed?: number;

  // Shipyard requirements (display only in Phase A)
  // Example: "Shipyard 20 (1)" => requiredShipyardLevel: 20, requiredOrbitalShipyardLevel: 1
  requiredShipyardLevel?: number;
  requiredOrbitalShipyardLevel?: number;

  // Tech prerequisites required empire-wide
  techPrereqs: UnitTechPrereq[];

  // Optional subtitle under name (shown in Units table). If omitted, UI may map a default.
  description?: string;
}

// Catalog populated per screenshots
export const unitsCatalog: Record<UnitKey, UnitSpec> = {
  fighter: {
    key: 'fighter',
    name: 'Fighter',
    creditsCost: 5,
...
export function getUnitsList(): UnitSpec[] {
  return Object.values(unitsCatalog);
}

export function getUnitSpec(key: UnitKey): UnitSpec | undefined {
