...
/**
 * Tests for HybridGameLoopService.processTechQueue (capacity-driven tech completion)
 * We call the private method via (hybridGameLoop as any).processTechQueue() to isolate behavior.
 */
...
import { hybridGameLoop } from '../../packages/server/src/services/hybridGameLoopService'

describe('HybridGameLoopService.processTechQueue', () => {
  const empireId = '64a7b2c3d4e5f6a7b8c9d0e1'
  const locationCoord = 'A00:00:00:00'
  const techKey: TechnologyKey = DB_FIELDS.EMPIRES.ENERGY
  ...
  it('promotes tech level to 1 and marks queue item completed', async () => {
    await (hybridGameLoop as any).processTechQueue()
