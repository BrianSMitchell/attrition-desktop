...
// Mocks for database models (minimal behaviors used by StructuresService.start)
...
// Mock TechService to avoid hitting getBaseLabTotal (which queries Building.find().select(DB_FIELDS.BUILDINGS.LEVEL))
...
describe('Capacity Parity — CapacityService and StructuresService', () => {
  beforeEach(() => {
    ...
    //  - TechService.getBaseLabTotal: Building.find(...).select(DB_FIELDS.BUILDINGS.LEVEL)
    BuildingFind.mockImplementation(() => ({
      select: jest.fn((sel: any) => {
        if (sel === DB_FIELDS.BUILDINGS.LEVEL) {
